...
local function bino(lhs,rhs,opt) vim.keymap.set('i',lhs,rhs,vim.tbl_extend('error',{buffer=true},opt or {})) end
autocmd('FileType',function()
    bino('ł','local ')
    bino('đ','function ')
    bino('®','return ')
...
autocmd('FileType',function ()
    bino('<tab>','"|<C-o>>>".repeat("<C-o>l",&tabstop)."<C-o>X"',{expr=true})
    bino('<S-tab>','repeat("<C-o>h",&tabstop)."<C-o><<"',{expr=true})

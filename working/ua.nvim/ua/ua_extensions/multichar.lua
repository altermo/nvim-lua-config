return {
    filter=function (o,conf,_,gmem)
        ...
        for newkey,opt in pairs(gmem) do
            ...
            if bool and opt.ext.rules and #opt.ext.rules~=0 then
                local rules=require'ua.memory'.load_extension('rules')
                bool=rules.check_rules(opt.ext.rules,o)
            end
            if bool and conf and vim.tbl_contains(conf,newkey) then
